...
export default function WebsiteBuilderPage() {
  ...
  if (!isFeatureAllowed) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] p-6 text-center">
        ...
        <h2 className="text-lg font-bold text-white">Website Builder Locked</h2>

        <p className="mt-1.5 max-w-sm text-sm text-[#64748B]">
          Your current{" "}
          <span className="font-semibold text-amber-400">{currentPlan.tier}</span>{" "}
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-[#94A3B8]">
            Design your dealership website, add tracking, and publish it live.
          </p>
        </div>

        {/* Tabs */}
        <div className="flex rounded-xl border border-[#1e2a4a] bg-[#111B33] p-1 self-start">
          <button
            onClick={() => setActiveTab("editor")}
            className={`flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold transition-colors ${
              activeTab === "editor" ? "bg-[#FC5E01] text-white" : "text-[#94A3B8] hover:text-white"
            }`}
          >
            <Layout size={15} />
            Editor
          ...
          <button
            ...
            className={`flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold transition-colors ${
              activeTab === "domain" ? "bg-[#FC5E01] text-white" : "text-[#94A3B8] hover:text-white"
            }`}
          >
            <Globe size={15} />
            Custom Domain
      ...
      {/* Content */}
      {activeTab === "editor" ? <WebsiteEditor /> : <DomainConnect />}
    </div>
  );
}