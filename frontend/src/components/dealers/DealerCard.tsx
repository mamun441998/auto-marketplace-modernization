...
export default function DealerCard({ dealer, index }: DealerCardProps) {
  const hasRating = typeof dealer.reviewCount === "number" && dealer.reviewCount > 0;
  ...
  return (
    ...
    >
      {/* Header: Logo/Avatar + Verified */}
      <div className="flex items-start justify-between">
        {dealer.logo ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={dealer.logo} alt={dealer.name} className="h-14 w-14 rounded-2xl object-cover border border-[#262626] bg-white" />
        ) : (
      ...
      {/* Rating (only if we have reviews) */}
      {hasRating ? (
        ...
        <div className="mt-3 text-xs font-semibold text-[#64748B]">New on MotoHave</div>
      )}

      {/* Specialties (only if present) */}
      {dealer.specialties && dealer.specialties.length > 0 && (
        <div className="mt-4 flex flex-wrap gap-1.5">
          {dealer.specialties.map((tag) => (
            <span key={tag} className="rounded-full border border-[#262626] bg-[#0A0A0A] px-2.5 py-1 text-[11px] font-medium text-[#94A3B8]">
              {tag}
            </span>
          ))}
        </div>
      ...
      <div className="mt-5 flex items-center justify-between border-t border-[#262626] pt-4">
        ...
        <Link
          href={dealer.slug ? `/s/${dealer.slug}` : `/inventory?dealer=${dealer.id}`}
